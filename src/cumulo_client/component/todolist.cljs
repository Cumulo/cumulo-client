...
(defn handle-add [state mutate] (fn [e dispatch] (dispatch :task/add state) (mutate "")))
  ...
  (fn [state mutate]
    (div
     ...
     (div
      {}
      (input
       {:style style-input, :event {:input (handle-input mutate)}, :attrs {:value state}})
      (div {:style style-button, :event {:click (handle-add state mutate)}} (text "Add")))
     (div
      {}
      (->> tasks
